...
replica_primary="$("${compose[@]}" exec -T mongo mongosh --quiet 'mongodb://127.0.0.1:27017/admin?directConnection=true' --eval 'process.stdout.write(String(db.adminCommand({hello:1}).isWritablePrimary === true))')"
test "$replica_primary" = "true"

product_id="66a000000000000000000002"
"${compose[@]}" exec -T mongo mongosh --quiet 'mongodb://127.0.0.1:27017/meow_matrix?directConnection=true' --eval "
...
auth_me="$(curl --fail-with-body --silent -b "$cookies" "$api_origin/api/v1/auth/me")"
auth_email="$(printf '%s' "$auth_me" | json_expr 'j.data.email')"
test "$auth_email" = "$email"

cart="$(curl --fail-with-body --silent \
  -b "$cookies" \
  -X PUT \
  -H 'Content-Type: application/json' \
  -H "Origin: $web_origin" \
...
checkout="$(curl --fail-with-body --silent \
  -b "$cookies" \
  -X POST \
  -H "Origin: $web_origin" \
...
stock_after_checkout="$(curl --fail-with-body --silent "$api_origin/api/v1/products/$product_id" | json_expr 'j.data.stock')"
test "$stock_after_checkout" = "3"

replay="$(curl --fail-with-body --silent \
  -b "$cookies" \
  -X POST \
  -H "Origin: $web_origin" \
  -H "Idempotency-Key: $idempotency_key" \
...
stock_after_replay="$(curl --fail-with-body --silent "$api_origin/api/v1/products/$product_id" | json_expr 'j.data.stock')"
test "$stock_after_replay" = "3"

orders="$(curl --fail-with-body --silent -b "$cookies" "$api_origin/api/v1/orders?limit=25&offset=0")"
...
for attempt in $(seq 1 50); do
  messages="$(curl --fail --silent "$mailpit_origin/api/v1/messages")"
...
curl --fail-with-body --silent -b "$cookies" \
...
persisted_order="$(curl --fail-with-body --silent -b "$cookies" "$api_origin/api/v1/orders/$order_id")"
...
deleted_status="$(curl --silent -o /dev/null -w '%{http_code}' -b "$cookies" "$api_origin/api/v1/files/$file_id/content")"
test "$deleted_status" = "404"

final_stock="$(curl --fail-with-body --silent "$api_origin/api/v1/products/$product_id" | json_expr 'j.data.stock')"
test "$final_stock" = "3"
